...
### Data Flow
```
Source Data (CSV) → AWS S3 → Snowflake (Staging) → Bronze Layer → Silver Layer → Gold Layer (One Big Table and Fact/Dimensions)
                                                           ↓              ↓           ↓
...
1. **Snowflake Account (create one if doesn't exist)**

2. **Python Environment**
   - Python 3.12 or higher
   - pip or uv package manager

3. **AWS Account (create one if doesn't exist) ** (for S3 storage)
